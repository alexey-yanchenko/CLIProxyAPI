...
// This fork counts thoughtsTokenCount towards completion_tokens, so a payload
// without candidatesTokenCount still reports the reasoning tokens rather than 0.
func TestConvertGeminiResponseToOpenAIIncludesCompletionTokensWhenCandidatesMissing(t *testing.T) {
	var param any
	...
	if !completionTokens.Exists() || completionTokens.Int() != 42 {
		t.Fatalf("completion_tokens = %s, want present with value 42. Output: %s", completionTokens.Raw, result[0])
	}
}

func TestConvertGeminiResponseToOpenAINonStreamIncludesCompletionTokensWhenCandidatesMissing(t *testing.T) {
