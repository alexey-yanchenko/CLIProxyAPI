...
func ConvertClaudeResponseToOpenAI(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	case "message_delta":
		...
		if usage := root.Get("usage"); usage.Exists() {
			promptTokens, completionTokens, totalTokens, cachedTokens, cacheCreationInputTokens := calculateClaudeUsageTokens(usage)
			...
			template, _ = sjson.SetBytes(template, "usage.prompt_tokens_details.cache_write_tokens", cacheCreationInputTokens)
		}
		return [][]byte{template}
