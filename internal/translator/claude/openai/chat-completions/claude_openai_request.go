						...
						if part.Get("type").String() == "text" {
							...
							if cacheControl := part.Get("cache_control"); cacheControl.Exists() {
								if cacheControl.Get("type").String() == "ephemeral" {
									textPart, _ = sjson.SetRawBytes(textPart, "cache_control", []byte(`{"type":"ephemeral"}`))
								}
							}
							out, _ = sjson.SetRawBytes(out, "system.-1", textPart)
						}
...
func convertOpenAIContentPartToClaudePart(part gjson.Result) string {
	...
	case "image_url":
		imagePart := []byte(convertOpenAIImageURLToClaudePart(part.Get("image_url.url").String()))

		if cacheControl := part.Get("cache_control"); cacheControl.Exists() {
			if cacheControl.Get("type").String() == "ephemeral" {
				imagePart, _ = sjson.SetRawBytes(imagePart, "cache_control", []byte(`{"type":"ephemeral"}`))
			}
		}
