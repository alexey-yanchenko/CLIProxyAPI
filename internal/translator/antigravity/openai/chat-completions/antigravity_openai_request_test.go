...
func TestConvertOpenAIRequestToAntigravityThinkingAliases(t *testing.T) {
	tests := []struct {
		...
		{
			// This fork enables thoughts by default when no reasoning_effort is given.
			name: "Missing summary intent enables thoughts by default",
			body: `{
		...
		{
			// The string value is discarded; this fork's default then applies.
